    ...
    for actors in series.values:
        for actor in actors:
            ...
            while index + 1 < len(actors):
                ...
                # Эта строка добавлена для того, чтобы решить проблему, описанную ниже. 
                # В первончальной версии ее не было.
                pairs.append(actors[index+1] + ' + ' + actor)
                #
                index += 1
    return pairs

# Допустим, одном фильме актеры идут в порядке A, B, C
